...
public class RichEditText extends EditText implements
    EditorActionModeListener , OnKeyListener {
  ...
  private static final HashSet<Effect<?>> KeyObservers=
	      new HashSet<Effect<?>>();
  ...
  public void registerKeyObserver(Effect<?> e){
	  KeyObservers.add(e);
  }
  public void removeKeyObserver(Effect<?> e){
	  KeyObservers.remove(e);
  }
	@Override
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		if ( event.getAction() == KeyEvent.ACTION_UP 
				&& keyCode == KeyEvent.KEYCODE_ENTER){
			for ( Effect<?> e : KeyObservers ){
				e.onKeyEnter(this);
			}
		}
	    if (keyboardShortcuts) {
